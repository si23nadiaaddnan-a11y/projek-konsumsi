...
const lokasiOptions = ["Bagging", "CCB", "Club House", "Departemen Riset", "Gedung 101-K", "Gedung Anggrek", " Gedung Bidding Center", "Gedung Contraction Office", "Gedung K3", "Gedung LC", "Gedung Maintanance Office", "Gedung Mawar", "Gedung Melati", "Gedung Purna Bhakti", "Gedung Pusat Administrasi", "Gedung RPK", "Gedung Saorga", "GH-B", "GH-C", "GPA Lt-3", "Gudang Bahan Baku", "Gudang Bulk Material", "Gudang Suku Cadang", "Jakarta", "Kantor SP2K", "Kebon Bibit", "Klinik PT HPH", "Kolam Pancing Type B", "Kolam Renang", "Kujang Kampioen Riset", "Laboraturium/Main Lab", "Lapang Basket Type B", "Lapang Futsal", "Lapang Sepak Bola Type E", "Lapang Tenis Type B", "Lapang Volly Type E", "Lapangan Helipad", "Lapangan Panahan", "Lapangan Volley", "Mekanik K1A", "Mekanik K1B", "Not Defined", "NPK-2", "Pos Selatan 01", "Posko Pengamanan Bawah", "Ruang Rapat NPK-1", "Ruang Rapat NPK-2", "Utility K-1A", "Wisma Kujang"];
...
const SearchableSelect: React.FC<SearchableSelectProps> = ({ options, value, onValueChange, placeholder, searchPlaceholder, notFoundMessage, className }) => {
...
// [BARU] Komponen untuk efek konfeti
const ConfettiPiece: React.FC = () => {
    const colors = ['#8b5cf6', '#a78bfa', '#d946ef', '#f472b6', '#fb7185'];
    const randomColor = colors[Math.floor(Math.random() * colors.length)];
    const randomX = Math.random() * 100;
    const randomDelay = Math.random() * 2;
    const randomDuration = 2 + Math.random() * 3;
    const randomRotation = -200 + Math.random() * 400;
...
const OrderHistory: React.FC<OrderHistoryProps> = ({ history, onDeleteRequest, onViewDetails, viewMode }) => {
    ...
    return (
        <div className="w-full mt-4 min-h-[50vh]">
            {!hasHistory ? (
               <div className="h-full flex items-center justify-center text-center text-muted-foreground rounded-xxl border-2 border-dashed">
...
const OrderFormContent: React.FC<OrderFormProps> = ({ initialData, onSubmit, onCancel, isSuccessful, setIsSuccessful }) => {
    ...
    // State untuk menyimpan opsi konsumsi yang tersedia
    const [availableKonsumsi, setAvailableKonsumsi] = useState<string[]>([]);

    // useEffect sekarang bergantung pada sesiWaktu dan tipeTamu
    useEffect(() => {
        const menusForTime = formData.sesiWaktu ? menuByTime[formData.sesiWaktu] || [] : [];
        const menusForGuest = formData.tipeTamu ? menuByGuestType[formData.tipeTamu] || [] : [];

        if (formData.sesiWaktu && formData.tipeTamu) {
            // Cari irisan antara menu berdasarkan waktu dan tipe tamu
            const finalOptions = menusForTime.filter(menu => menusForGuest.includes(menu));
            setAvailableKonsumsi(finalOptions);

            // Reset jika pilihan saat ini tidak lagi valid
            if (!finalOptions.includes(formData.jenisKonsumsi)) {
                setFormData(prev => ({ ...prev, jenisKonsumsi: '', satuan: '' }));
            }
        } else {
            // Kosongkan opsi jika salah satu belum dipilih
            setAvailableKonsumsi([]);
            if (formData.jenisKonsumsi !== '') {
                 setFormData(prev => ({ ...prev, jenisKonsumsi: '', satuan: '' }));
            }
        }
    }, [formData.sesiWaktu, formData.tipeTamu]);

    // useEffect untuk mengatur Satuan berdasarkan Jenis Konsumsi
    useEffect(() => {
        if (formData.jenisKonsumsi && unitByConsumption[formData.jenisKonsumsi]) {
            const newUnit = unitByConsumption[formData.jenisKonsumsi];
            if (formData.satuan !== newUnit) {
                // Gunakan setFormData untuk menghindari loop tak terbatas dengan handleChange
                setFormData(prev => ({ ...prev, satuan: newUnit }));
            }
        }
    }, [formData.jenisKonsumsi]);
                ...
                <div className="relative overflow-hidden">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                        ...
                        <CardFooter className="flex-col sm:flex-row justify-center gap-4 p-6 bg-slate-50 dark:bg-slate-900/50">
                            <Button variant="outline" onClick={onCancel} className="border-violet-300 hover:bg-violet-100 hover:text-violet-700">
                                <ChevronUp className="mr-2 h-4 w-4" />Kembali ke Riwayat
                            </Button>
                        ...
                        <CardContent className="grid gap-6 p-6">
                            ...
                            <div className="space-y-4 pt-4 border-t">
                                ...
                                <div className="grid sm:grid-cols-3 gap-4">
                                    <div className="space-y-2"><Label htmlFor="lokasiPengiriman">Lokasi Pengiriman</Label>
                                    <SearchableSelect
                                        value={formData.lokasiPengiriman}
                                        onValueChange={(value) => handleChange('lokasiPengiriman', value)}
                                        options={allOptions.lokasiOptions}
                                        placeholder="Pilih lokasi pengiriman"
                                        searchPlaceholder="Cari lokasi..."
                                        notFoundMessage="Lokasi tidak ditemukan."
                                        className={cn({ "border-red-500": errors.lokasiPengiriman })}
                                    />
                                    {errors.lokasiPengiriman && <p className="text-xs text-red-600">{errors.lokasiPengiriman}</p>}</div>
                                    <div className="space-y-2"><Label htmlFor="sesiWaktu">Waktu</Label><Select onValueChange={(value) => handleChange('sesiWaktu', value)} value={formData.sesiWaktu}><SelectTrigger id="sesiWaktu"><SelectValue placeholder="Pilih waktu" /></SelectTrigger><SelectContent>{allOptions.sesiWaktuOptions.map((opt, index) => <SelectItem key={`${opt}-${index}`} value={opt}>{opt}</SelectItem>)}</SelectContent></Select></div>
                                ...
                                <div className="grid sm:grid-cols-3 gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <Select 
                                            onValueChange={(value) => handleChange('jenisKonsumsi', value)} 
                                            value={formData.jenisKonsumsi}
                                            disabled={!formData.sesiWaktu || !formData.tipeTamu}
                                        >
                                            <SelectTrigger id="jenisKonsumsi" className={cn({ "border-red-500": errors.jenisKonsumsi })}>
                                                <SelectValue placeholder={!formData.sesiWaktu || !formData.tipeTamu ? "Pilih Konsumsi" : "Pilih jenis"} />
                                            </SelectTrigger>
                                            <SelectContent>
                                                {availableKonsumsi.map(opt => <SelectItem key={opt} value={opt}>{opt}</SelectItem>)}
                                            </SelectContent>
                                        </Select>
                                        {errors.jenisKonsumsi && <p className="text-xs text-red-600">{errors.jenisKonsumsi}</p>}
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Select 
                                            onValueChange={(value) => handleChange('satuan', value)} 
                                            value={formData.satuan}
                                            disabled={!formData.jenisKonsumsi}
                                        >
                                            <SelectTrigger id="satuan" className={cn({ "border-red-500": errors.satuan })}>
                                                <SelectValue placeholder="Pilih satuan" />
                                            </SelectTrigger>
                                            <SelectContent>
                                                {allOptions.satuanOptions.map(opt => <SelectItem key={opt} value={opt}>{opt}</SelectItem>)}
                                            </SelectContent>
                                        </Select>
                                        {errors.satuan && <p className="text-xs text-red-600">{errors.satuan}</p>}
                                    </div>
                                </div>
